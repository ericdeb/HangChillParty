...
g.nextSibling.appendChild(document.createTextNode(h==1?"digg":"diggs"));if(j.id){i.firstChild.firstChild.className="db-container"
...
g.setAttribute("type","text/javascript");g.setAttribute("async","true");g.setAttribute("src",j);i?h.insertBefore(g,i):h.appendChild(g)
...
if(f.ie6){k.className="db-ie db-ie6"}}h=document.createElement("SPAN");h.className="db-container db-submit";
g=document.createElement("SPAN");g.className="db-body";j=document.createElement("SPAN");j.className="db-count";
m=document.createElement("SPAN");m.className="db-copy";i=document.createElement("A");i.className="db-anchor";
d.appendChild(k);k.appendChild(h);h.appendChild(g);g.appendChild(j);g.appendChild(m);g.appendChild(i);
i.appendChild(document.createTextNode("digg"))},I=function(h){var g=[];for(prop in h){if(h[prop]!==null){g.push(prop+"="+h[prop])
}}return"?"+g.join("&")},T=function(h){var g;for(prop in h){if(O[prop]){g=h[prop];delete h[prop];h[O[prop]]=g
}}return h},U=function(i,h){if(G){var k={source:"button",submitted:!!i.id,event:h,diggs:i.diggs,storyId:i.id||0,buttons:S},l=T(k),j=M.analytics+I(l),g=Math.floor(Math.random()*100)+1;
if(g<=W[h]){new Image().src=j}}},Z=function(g){return g.replace(/^\s+|\s+$/g,"")},N,K,X,S=0,P,C=function(){H&&H();
d||E();P();if(!X){N=setTimeout(C,500)}},A=function(){var g=['<a class="DiggThisButton'];if(window.digg_skin=="compact"){g.push(' DiggCompact"')
}else{if(window.digg_skin=="icon"){g.push(' DiggIcon"')}else{g.push('"')}}if(window.digg_title){g.push(' href="',M.submit,"url=",escape(digg_url||DIGG_URL||location.href).replace("+","%2b","g"),"&title=",escape(digg_title),'"')
}else{if(window.digg_url||window.DIGG_URL){g.push(' href="',M.submit,"url=",escape(digg_url||DIGG_URL).replace("+","%2b","g"),'"')
...
for(;j<g;j++){k=" "+h[j].className+" ";if(k.indexOf(" DiggThisButton ")>=0){F(h[j]);S++}}}}C();if(window.addEventListener){window.addEventListener("load",J,true)
}else{if(window.attachEvent){window.attachEvent("onload",J)}else{K=window.onload;window.onload=K?function(){K();
J()}:J}}return{writeLink:A,addButtons:C,collectDiggs:function(k){var h=b[escape(k.url).replace("+","%2b","g")],g=h.length,j=0;
for(;j<g;j++){h[j](k)}}}})()};